...
DB_PATH           = os.path.join(BASE_DIR, "extraction_results_v6.duckdb")
...
_NOTARY_BEFORE_RE = re.compile(
    r"(?:"
    r"ante\s+(?:el\s+|la\s+)?notar[ií][oa]\s+(?:p[uú]blic[oa]\s+)?(?:titular|suplente|interino|reemplazante)?\s*(?:de\s+\w+\s*)?(?:don\s+|do[ñn]a\s+)?"
    r"|notar[ií][oa]\s+(?:p[uú]blic[oa]\s+)?(?:titular|suplente|interino|reemplazante)?\s*(?:,\s*)?(?:de\s+\w+\s+)?\s*(?:don\s+|do[ñn]a\s+)?"
    r"|(?:suplente|subrogante|interino|reemplazante)\s+de\s+(?:don\s+|do[ñn]a\s+)"
    r"|conservador\s+(?:de\s+)?\w+\s+(?:don\s+|do[ñn]a\s+)?"
    r"|ante\s+m[ií]\s*(?:,?\s*suplente\s*)?(?:,\s*)?"
    r"|ante\s+(?:el\s+|mi\s+)?suplente\s+"
    r")$",
    re.IGNORECASE
)
...
_HISTORICAL_NOTARY_RE = re.compile(
    ...
    r"|(?:ante\s+(?:el\s+|la\s+)?notar[ií][oa]\s+(?:p[uú]blic[oa]\s+)?(?:de\s+\w+\s+)?(?:de\s+)?(?:don\s+|do[ñn]a\s+)?)"  
    r"|(?:notar[ií]a\s+(?:de\s+\w+\s+)?(?:de\s+)?(?:don\s+|do[ñn]a\s+)?)"
    r")$",
    re.IGNORECASE
)
...
_TARGET_MARKERS_MODIFICARON = re.compile(
    r"modificaron\s+(?:los\s+estatutos\s+sociales\s+de\s+)?" + _Q + r"?([^\n\"\u201c\u201d\u201e]+?(?:LTDA\.?|LIMITADA|SpA|S\.?A\.?|E\.?I\.?R\.?L\.?))" + _Q + r"?",
    re.IGNORECASE
)
...
def find_rut_for_entity(e_start: int, e_end: int, ruts: list, text: str,
    ...
    for idx, (rut_str, r_start, r_end) in enumerate(ruts):
        ...
        if "rol" in ctx_before and "tributario" in ctx_before:
            score += 200 # Heavy bonus for companies
        elif "cédula" in ctx_before or "identidad" in ctx_before or "run" in ctx_before:
            ...
            if prefer_col_unico:
                ...
                score += 100
        elif re.search(r"\brut\b\s*[:#no.]*\s*$", ctx_before):
...
def is_notary_context(text: str, e_start: int, e_end: int) -> bool:
    ...
    # Check for "ante mí" closely preceding
    if re.search(r"ante\s+m[ií]\s*,?\s*$", before_text):
        return True
...
def run_inference(model, docs: list) -> list:
    ...
    with torch.inference_mode():
        preds = model.batch_predict_entities(sorted_texts, NER_LABELS, threshold=NER_THRESHOLD)
    ...
    for sorted_i, original_i in enumerate(order):
        result[original_i] = preds[sorted_i]
        ...
        if target_ent and subject_name != "UNKNOWN_TARGET":
            ...
            for m_start, m_end in name_mentions:
                rut, ridx = find_rut_for_entity(m_start, m_end, ruts, text, 
                                                max_dist=300, consumed=consumed_ruts,
